...
app.get('/new', function(request, response) {
	if (request.session.loggedin) {
		response.sendFile(path.join(__dirname + '/my/new.html'));
	} else {
		response.send('Please login to view this page!');
		response.end();
	}
...
app.get('/sale', function(request, response) {
	if (request.session.loggedin) {
		response.sendFile(path.join(__dirname + '/my/sale.html'));
	} else {
		response.send('Please login to view this page!');
		response.end();
	}
...
app.get('/shoes', function(request, response) {
	if (request.session.loggedin) {
		response.sendFile(path.join(__dirname + '/my/shoes.html'));
	} else {
		response.send('Please login to view this page!');
		response.end();
	}
